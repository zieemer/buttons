...
(function(_) {
  ...
  function onRedirect(info) {
      chrome.tabs.get(info.tabId, function (tab) {
          // ignore unsuported tab urls like chrome://, about: and chrome.google.com/webstore - these are banned by google.
          if (!tab || !AvastWRC.bs.checkUrl(tab.url)) {
              return;
          }
...
(function(AvastWRC, chrome, _) {
  ...
  var EDITIONS_CONFIG = [
    // AOS
    { extType: AvastWRC.EXT_TYPE_AOS,  callerId: 1020, reqUrlInfoServices: 0x000F, extVer: 15, dataVer: 15 }, // AOS (WR, P, B, SC)
    // ABOS
    { extType: AvastWRC.EXT_TYPE_ABOS, callerId: 7020, reqUrlInfoServices: 0x000F, extVer: 15, dataVer: 15 } // AOS (WR, P, B, SC)
  ];
